...
@Component({
  selector: 'app-daff',
  templateUrl: './daff.page.html',
  styleUrls: ['./daff.page.scss'],
  standalone: true,
  imports: [
    IonContent,
    IonHeader,
...
export class DaffPage implements OnInit {
  currentGame: number = 1;
  pieces: any[] = [];
  dragIndex: number | null = null;

  // ===== Juego 1: Adivina la Palabra =====
  words = ['KOO', 'GABY', 'TATA', 'DAFF', 'BTS'];
  currentWord: string = '';
  guessedWord: string[] = [];
      ...
      if (this.cardIcons[first] === this.cardIcons[second]) {
        ...
        if (this.matchedPairs === this.cardIcons.length / 2) {
          ...
          setTimeout(() => {
            this.showAlert(
              '¡Felicidades!',
              'Has completado todos los juegos Daff -`♡´-'
            );
            this.currentGame = 4;
          }, 1000);
        }
